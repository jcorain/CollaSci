...
def add_popup(parent, connection, table_name, parent_parent):
    ...
    if table_name == 'user':
        tab_user.UserAdd(popup, connection, parent_parent)
...
class AddButton(tk.Frame):
    def __init__(self, parent, connection, table_name, parent_parent):
        # define the frame for the delete buttton
        tk.Frame.__init__(self, parent)
        
        # define the first label holding the already defined users 
        
        self.label_add = tk.Button(parent, text = 'Add a column', command = lambda : add_popup(parent, connection, table_name, parent_parent))
        self.label_add.pack()